...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($user) {
    ...
    if (password_verify($password_input, $user['password'])) {
      ...
      // Registra o login na tabela session_logs
      try {
        $stmt_log = $pdo->prepare("INSERT INTO session_logs (user_id, login) VALUES (:user_id, NOW())");
        $stmt_log->execute(['user_id' => $user['id']]);
        $_SESSION['session_log_id'] = $pdo->lastInsertId(); // Salva o ID do log para atualizar no logout
      } catch (Exception $e) {
        // Se falhar ao registrar o log, continua o login normalmente
        error_log("Erro ao registrar login: " . $e->getMessage());
      }

      if ($user['user_type'] == 'A') {
        // Se o usuário for do tipo Administrador, redireciona para a página de administração              
        header('Location: ../Master/master.html', true, 302);
        exit;
      } else {
  ...
  <div class="container" id="container">
    ...
    <div class="form-container sign-in-container">
      <form action="login.php" method="post">
        ...
        <button>Logar</button>
        <br>
        <div style="color: rgb(194, 181, 166);">
          <p> Não tem conta? </p> <a href="Cadastro.php" class="ghost">Registre-se</a>
        </div>
        <?php echo $mensagem; ?>
